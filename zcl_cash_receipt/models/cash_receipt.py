...
class CashReceipt(models.Model):
    ...
    line_ids = fields.One2many('cash.receipt.line', 'item_id', string='Line ids')
...
class CashReceiptLine(models.Model):
    _name = 'cash.receipt.line'
    _description = "Cash Receipt Line"

    item_id = fields.Many2one('cash.receipt',)
    ...
    def open_invoice_wizard(self):
        reference_wizard_data = self.env['customer.invoice.wizard']
        reference_wizard_payment_panding = self.env['customer.invoice.wizard'].search([('cash_receipt_line.id', '=', self.id), ('payment_done', '=', False)]).unlink()
        reference_wizard_payment_done = self.env['customer.invoice.wizard'].search([('cash_receipt_line.id', '=', self.id), ('payment_done', '=', True)])
        if not reference_wizard_payment_done:
            ...
            if posted_invoices:
                ...
                reference_data = {
                    'cash_receipt_line': self.id,
                    'line_amount': self.amount,
                    'total_amount': self.amount,
                    'line_ids': line_ids,
                }
                reference_wizard_data = self.env['customer.invoice.wizard'].sudo().create(reference_data)

            return {
                'name': 'Invoice Reference',
                'type': 'ir.actions.act_window',
                'res_model': 'customer.invoice.wizard',
                'view_mode': 'form',
                'view_type': 'form',
                'target': 'new',
                'res_id': reference_wizard_data.id,
                'domain': [('cash_receipt_line', '=', self.id)],
            }
